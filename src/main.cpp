...
// Création des objets LVGL pour les raquettes, la balle et les scores
lv_obj_t *paddle1;
lv_obj_t *paddle2;
lv_obj_t *ball;
lv_obj_t *score_label1;
lv_obj_t *score_label2;
lv_obj_t *winner_label = NULL;
...
void init_ui() {
    ...
    // Création de la balle et la placer entre les raquettes (au milieu)
...
void display_winner(const char* winner) {
    if (winner_label == NULL) {
        winner_label = lv_label_create(lv_scr_act());
        lv_obj_set_style_text_font(winner_label, &lv_font_montserrat_14, 0);
    }
    lv_label_set_text_fmt(winner_label, "%s  gagne", winner);
    lv_obj_align(winner_label, LV_ALIGN_CENTER, 0, 0);
    threadLvgl.unlock();
    ThisThread::sleep_for(2000ms); // Attendre 2 secondes avant de réinitialiser le jeu
    clear_winner(); // Effacer le texte de victoire après 2 secondes
    threadLvgl.lock();
}
...
// Fonction mise à jour de l'état du jeu
void update_game_state() {
    ...
    // Rebondir la balle sur les murs de l'écran
    ...
    if (ball_x <= 0) {
        ...
        update_score_labels(); // Mettre à jour les scores
